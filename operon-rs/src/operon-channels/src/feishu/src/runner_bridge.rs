// runner_bridge.rs — Operon SessionRunner bridge for Feishu / Lark channel.
//
// Hey friend! This file bridges Feishu incoming messages to `operon_session::SessionRunner`.
//
// Flow per inbound turn:
//   1. Check if first-time user -> auto-send onboarding documentation over Feishu.
//   2. Provision user workspace (`~/.operon/sessions/feishu/<user_id>/`) & system instructions.
//   3. Compute JSON session store path (`~/.operon/sessions/feishu/<user_id>/<session_id>.json`).
...
//      - `TextDelta` / `Done`: send final formatted response payload back to Feishu.
...
use crate::outbound::FeishuOutboundMessage;
use crate::router::FeishuRouter;
...
/// Bridge that drives `SessionRunner` for a specific Feishu user and sends output over the Feishu outbound channel.
pub struct SessionRunnerBridge {
    app_config: AppConfig,
    workspace_manager: FeishuWorkspaceManager,
    outbound_tx: mpsc::Sender<FeishuOutboundMessage>,
    router: Option<Arc<FeishuRouter>>,
...
impl SessionRunnerBridge {
    ...
    /// Creates a new `SessionRunnerBridge` wired with `FeishuRouter` to support turn cancellation on `/new`.
    pub fn with_router(
        app_config: AppConfig,
        workspace_manager: FeishuWorkspaceManager,
        outbound_tx: mpsc::Sender<FeishuOutboundMessage>,
    ...
    /// Creates a new `SessionRunnerBridge` wired with `FeishuRouter` and an external `SessionEventHook`.
    ...
    /// Auto-sends first-time onboarding documentation message over Feishu.
    pub async fn send_onboarding(
        ...
        let msg = FeishuOutboundMessage::new_reply(receive_id, text, reply_to_message_id);
        let _ = self.outbound_tx.send(msg).await;
        Ok(())
    }

    /// Process a turn for a user message over Feishu.
    pub async fn process_turn(
        ...
        if is_first_time {
            let _ = self.send_onboarding(user_id.as_str(), message_id.clone()).await;
        }
        ...
        let store = SessionStore::open(&store_path)
            .await
            .map_err(|e| FeishuError::Session(e.to_string()))?;
        ...
        info!(
            "Feishu session {} for user {}: is_new={}, history_turns={}, turn_index={}, last_token_count={:?}",
...
async fn forward_session_events_to_outbound(
    ...
    while let Some(event) = event_rx.recv().await {
        ...
        match event {
            SessionEvent::ApprovalRequired { ref tool, .. } => {
                ...
                let out = FeishuOutboundMessage::new_reply(
                    receive_id,
                    &msg,
            ...
            SessionEvent::Done => {
                send_final_text(receive_id, reply_to_message_id.clone(), outbound_tx, &accumulated_text).await;
                terminal_event_seen = true;
                break;
            }
            SessionEvent::Error { message } => {
                error!("SessionRunner error for Feishu user {}: {}", receive_id, message);
                let err_out = FeishuOutboundMessage::new_reply(
                    receive_id,
            ...
            other => {
                tracing::debug!(
                    ?other,
                    "SessionEvent variant intentionally ignored for Feishu forwarding"
                );
            }
        }
    }
    ...
    if !terminal_event_seen {
        send_final_text(receive_id, reply_to_message_id, outbound_tx, &accumulated_text).await;
    }
}
...
async fn send_final_text(
    ...
    if !trimmed.is_empty() {
        let msg = FeishuOutboundMessage::new_reply(receive_id, trimmed, reply_to_message_id);
...
mod tests {
    ...
    async fn forwards_final_text_as_soon_as_done_arrives() {
        let (outbound_tx, mut outbound_rx) = mpsc::channel::<FeishuOutboundMessage>(10);
        ...
        assert_eq!(received.text, "Hello from Feishu!");
