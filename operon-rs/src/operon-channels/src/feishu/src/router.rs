// router.rs — Message routing, command parsing, and role resolution for Feishu / Lark.
//
// Hey friend! This module inspects incoming Feishu / Lark messages, classifies the sender's role
...
use crate::types::{ChatId, FeishuMessage, UserId};
use operon_events::SessionCommand;
use operon_policy::CallerRole;

/// Represents an active session record for a Feishu user.
#[derive(Debug, Clone)]
pub struct ActiveSession {
    /// Unique session ID formatted as `fs-<hex_timestamp>`.
...
/// Outcome of routing an inbound Feishu message.
#[derive(Debug, Clone)]
...
impl FeishuRouter {
    /// Creates a new `FeishuRouter` with the given configuration.
    ...
    pub async fn route(&self, msg: &FeishuMessage) -> RouteOutcome {
            ...
            || trimmed_text.to_lowercase().starts_with("/new ")
        {
            ...
            let old_role = if let Some(existing) = sessions.get(&user_id) {
                if let Some(ref cmd_tx) = existing.cmd_tx {
                    info!(
                        "Sending SessionCommand::Cancel to running Feishu turn for user {}",
                        user_id
                    );
            ...
            if let Some(r) = old_role {
                if r != new_role {
                    info!(
                        "Feishu user {} role changed from {:?} to {:?} across /new boundary",
                        user_id, r, new_role
                    );
                }
            }
    ...
    /// Generates a new unique session ID with prefix `fs-`.
    fn generate_session_id() -> String {
        let epoch_millis = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|d| d.as_millis())
            .unwrap_or(0);
        format!("fs-{:x}", epoch_millis)
    }
}
