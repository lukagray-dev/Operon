...
use crate::dto::{
    BranchInfo, CommitResult, GitDiffStats, GitGraphCommit, RepoEntry, RepositoryDiff,
};
...
pub async fn get_diff_stats_async(workspace_root: PathBuf) -> Result<GitDiffStats, DiffError> {
    task::spawn_blocking(move || status::get_diff_stats(workspace_root)).await?
}
...
pub async fn get_diff_details_async(workspace_root: PathBuf) -> Result<RepositoryDiff, DiffError> {
    task::spawn_blocking(move || status::get_diff_details(workspace_root)).await?
}

/// Async non-blocking wrapper for `stage::stage_file`.
pub async fn stage_file_async(
    workspace_root: PathBuf,
    relative_path: String,
) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::stage_file(workspace_root, &relative_path)).await?
}

/// Async non-blocking wrapper for `stage::unstage_file`.
pub async fn unstage_file_async(
    workspace_root: PathBuf,
    relative_path: String,
) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::unstage_file(workspace_root, &relative_path)).await?
}

/// Async non-blocking wrapper for `stage::revert_file`.
pub async fn revert_file_async(
    workspace_root: PathBuf,
    relative_path: String,
) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::revert_file(workspace_root, &relative_path)).await?
}
...
pub async fn stage_all_files_async(workspace_root: PathBuf) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::stage_all_files(workspace_root)).await?
}
...
pub async fn revert_all_files_async(workspace_root: PathBuf) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::revert_all_files(workspace_root)).await?
}

/// Async non-blocking wrapper for `stage::discard_all_including_untracked`.
pub async fn discard_all_including_untracked_async(
    workspace_root: PathBuf,
) -> Result<(), DiffError> {
    task::spawn_blocking(move || stage::discard_all_including_untracked(workspace_root)).await?
}
...
pub async fn commit_async(
    ...
    task::spawn_blocking(move || commit::commit_workspace(workspace_root, &message, amend)).await?
}
...
pub async fn current_branch_async(workspace_root: PathBuf) -> Result<BranchInfo, DiffError> {
    task::spawn_blocking(move || branch::current_branch_workspace(workspace_root)).await?
}
...
pub async fn list_branches_async(workspace_root: PathBuf) -> Result<Vec<BranchInfo>, DiffError> {
    task::spawn_blocking(move || branch::list_branches_workspace(workspace_root)).await?
}
...
pub async fn switch_branch_async(workspace_root: PathBuf, name: String) -> Result<(), DiffError> {
    task::spawn_blocking(move || branch::switch_branch_workspace(workspace_root, &name)).await?
}
...
pub async fn delete_branch_async(workspace_root: PathBuf, name: String) -> Result<(), DiffError> {
    task::spawn_blocking(move || branch::delete_branch_workspace(workspace_root, &name)).await?
}
...
pub async fn rename_branch_async(
    ...
    task::spawn_blocking(move || {
        branch::rename_branch_workspace(workspace_root, &old_name, &new_name)
    })
    .await?
}
...
pub async fn fetch_async(workspace_root: PathBuf, remote_name: String) -> Result<(), DiffError> {
    task::spawn_blocking(move || remote::fetch_workspace(workspace_root, &remote_name)).await?
}
...
pub async fn discover_workspace_repos_async(
    workspace_root: PathBuf,
) -> Result<Vec<RepoEntry>, DiffError> {
    task::spawn_blocking(move || {
