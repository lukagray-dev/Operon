...
pub fn push(repo: &Repository, remote_name: &str, branch: &str) -> Result<(), DiffError> {
    ...
    remote
        ...
        .map_err(|e| {
            DiffError::RemoteAuth(format!("Push to '{remote_name}/{branch}' failed: {e}"))
        })?;
...
pub fn pull(repo: &Repository, remote_name: &str, branch: &str) -> Result<(), DiffError> {
    ...
    let fetch_head = repo.find_reference(&remote_ref_name).map_err(|_| {
        DiffError::BranchNotFound(format!(
            "Remote tracking branch '{remote_ref_name}' not found"
        ))
    })?;
    ...
    if analysis.is_fast_forward() {
        ...
        head_ref.set_target(
            target_oid,
            &format!("Fast-forward pull from {remote_name}/{branch}"),
        )?;
...
pub fn fetch_workspace<P: AsRef<Path>>(
    workspace_root: P,
    remote_name: &str,
) -> Result<(), DiffError> {
    let repo = discover_repository(workspace_root)?;
