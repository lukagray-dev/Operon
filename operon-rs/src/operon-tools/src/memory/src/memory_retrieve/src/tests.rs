...
#[tokio::test]
async fn test_memory_retrieve_success() {
    use sqlx::Row;
    ...
    let row = sqlx::query("INSERT INTO memories (content) VALUES ('retrieve test content') RETURNING id")
        .fetch_one(&mut conn)
        .await
        .unwrap();
    ...
    if let operon_context_normalize::tools::ToolContent::Text(text) = result.content {
        ...
        assert!(text.contains("retrieve test content"));
    } else {
