...
async fn test_memory_delete_success() {
    ...
    let row = sqlx::query("INSERT INTO memories (content) VALUES ('test memory content') RETURNING id")
        .fetch_one(&mut conn)
        .await
        .unwrap();
    ...
    if let operon_context_normalize::tools::ToolContent::Text(text) = result.content {
        assert!(text.contains(&format!("Memory with ID {} deleted successfully.", id)));
    } else {
