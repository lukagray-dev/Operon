...
async fn test_delete_existing_succeeds() {
    ...
    let mem = store
        .add("To be deleted".to_string(), vec![])
        .await
        .unwrap();

    let result = execute(ToolCallId("d1".to_string()), json!({"id": mem.id}), &store)
        .await
        .unwrap();
...
async fn test_delete_remaining_count_decreases() {
    ...
    let result = execute(ToolCallId("d2".to_string()), json!({"id": m1.id}), &store)
        .await
        .unwrap();
...
async fn test_delete_not_found_returns_error() {
    ...
    let result = execute(ToolCallId("d3".to_string()), json!({"id": "99999"}), &store)
        .await
        .unwrap();
...
async fn test_delete_memory_removed_from_get() {
    ...
    execute(ToolCallId("d5".to_string()), json!({"id": mem.id}), &store)
        .await
        .unwrap();
...
async fn test_delete_missing_id_returns_parse_error() {
    let (store, _tmp) = fresh_store().await;
    let err = execute(ToolCallId("d6".to_string()), json!({}), &store).await;
...
async fn test_delete_with_progress() {
    let (store, _tmp) = fresh_store().await;
    let mem = store
        .add("Progress delete".to_string(), vec![])
        .await
        .unwrap();
