...
Format:
<bash path="[working_directory]" command="[shell_command]" timeout="[milliseconds]">
...
- `command` (required attribute): The shell command to run.
- `timeout` (optional attribute): Kill command after specified milliseconds (default: 30 minutes).
