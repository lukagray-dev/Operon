        ...
        let run_load = async {
            ...
            if has_key || is_ollama {
                if let Ok(result) = operon_rs::discover_models(provider_enum, api_key.expose(), base_opt).await {
                    let mut discovered_models_ids: Vec<String> = result.models.iter().map(|m| m.model_id.clone()).collect();
                    if !discovered_models_ids.is_empty() {
                        ...
                        if !active_model.is_empty() {
                            if let Some(pos) = discovered_models_ids.iter().position(|m| m == &active_model) {
                                discovered_models_ids.remove(pos);
                            }
                            discovered_models_ids.insert(0, active_model.clone());
                        }

                        let slint_discovered: Vec<SharedString> = discovered_models_ids.into_iter().map(SharedString::from).collect();
                        let window_weak_clone = window_weak.clone();
                        let _ = slint::invoke_from_event_loop(move || {
                            if let Some(win) = window_weak_clone.upgrade() {
            ...
            let run_save = async {
                ...
                let (context_window, max_tokens) = {
                    ...
                    if let Some(m) = cached.iter().find(|m| m.model_id == selected_model_str) {
                        ...
                        (app_config.provider.model.context_window, app_config.provider.model.max_tokens)
                    }
                };
                ...
                let _ = slint::invoke_from_event_loop(move || {
                    if let Some(win) = win_weak.upgrade() {
                        ...
                        let utilization = if context_window > 0 { used as f32 / context_window as f32 } else { 0.0 };
                        win.set_context_usage(utilization);
                        let context_text = crate::main_content::input::context::format_tokens(used, context_window as i32);
