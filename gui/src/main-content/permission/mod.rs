...
pub fn append_approval_required(state: &mut ResponseState, id: &str, tool: &str, path: &str, reason: &str, args_json: &str) {
    state.flush_text();
...
pub fn append_permission_denied_event(state: &mut ResponseState, _tool: &str, _path: &str, _reason: &str) {
