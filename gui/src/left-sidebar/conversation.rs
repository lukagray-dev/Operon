...
pub async fn load_session_history(
    session_id: &str,
) -> anyhow::Result<(String, Vec<(bool, String, Vec<ParsedMarkdownItem>)>, usize, Option<usize>)> {
    ...
    // Get title
    let first_msg = store.get_first_user_message_text(session_id).await.ok().flatten();
    let title = crate::main_content::title::determine_session_title(first_msg.as_deref(), "New Chat");
    ...
    let last_token_count = store.get_last_token_count(session_id).await.ok().flatten().unwrap_or(0);
