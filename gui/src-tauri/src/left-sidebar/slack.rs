//! Slack channel session queries for the Left Sidebar.

use crate::left_sidebar::types::{SidebarConversationDto, SidebarProjectDto};
use std::path::PathBuf;

/// Query Slack user IDs and session JSON files from disk and construct SidebarProjectDto items.
#[tauri::command]
pub async fn query_slack_contacts() -> Result<Vec<SidebarProjectDto>, String> {
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("."));
    let base_sessions = home.join(".operon").join("sessions").join("slack");
