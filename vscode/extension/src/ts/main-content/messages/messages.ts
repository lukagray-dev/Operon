// Chat Messages Controller & Ultra-Smooth 60FPS DOM Stream Renderer for VS Code
...
function insertBlockInRow(row: HTMLElement, newEl: HTMLElement, blockIdx: number): void {
  ...
  for (const child of children) {
    ...
    if (idxAttr !== null) {
      ...
      child.classList.contains('assistant-controls-container') ||
      child.classList.contains('assistant-action-bar') ||
      child.classList.contains('turn-separator')
    ) {
      insertBeforeEl = child;
      break;
    }
...
function isChannelSession(sessionId: string | null): boolean {
  ...
  return sessionId.startsWith('wa-') || sessionId.startsWith('tg-');
}
...
export function initMessages(): void {
  ...
  sidebarState.subscribe(async () => {
    ...
    inputState.setReadOnly(isChannel, isChannel ? 'Channel conversation (read-only in VS Code)' : '');
