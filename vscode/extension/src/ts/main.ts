...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const copyWithFeedback = async (text: string, iconElem: HTMLElement | null, checkClass: string, originalClass: string) => {
    try {
  ...
  const appendUserMessage = (text: string) => {
    ...
    const actions = row.querySelector<HTMLElement>('.user-message-actions')!;
    const copyBtn = row.querySelector<HTMLButtonElement>('.btn-copy-user');
    const copyIcon = copyBtn?.querySelector<HTMLElement>('.ui-icon') || null;
    const editBtn = row.querySelector<HTMLButtonElement>('.btn-edit-user');
    ...
    editBtn?.addEventListener('click', () => {
      if (row.querySelector('.user-edit-container')) return;
      ...
      document.querySelectorAll<HTMLElement>('.user-edit-container').forEach((el) => {
        ...
        if (parentRow) {
          const b = parentRow.querySelector<HTMLElement>('.user-message-bubble');
          const a = parentRow.querySelector<HTMLElement>('.user-message-actions');
      ...
      const textarea = editContainer.querySelector<HTMLTextAreaElement>('.user-edit-textarea')!;
      const cancelBtn = editContainer.querySelector<HTMLButtonElement>('.btn-user-edit-cancel')!;
      const saveBtn = editContainer.querySelector<HTMLButtonElement>('.btn-user-edit-save')!;
  ...
  const appendAssistantMessage = (userPrompt: string) => {
    if (!messagesContainer) return;
    ...
    // Hook copy action
    const copyBtn = row.querySelector<HTMLButtonElement>('.btn-asst-copy');
    const copyIcon = copyBtn?.querySelector<HTMLElement>('.ui-icon') || null;
    copyBtn?.addEventListener('click', () => {
    ...
    const likeBtn = row.querySelector<HTMLButtonElement>('.btn-asst-like');
    const dislikeBtn = row.querySelector<HTMLButtonElement>('.btn-asst-dislike');
    const controlsContainer = row.querySelector<HTMLElement>('.assistant-controls-container');
